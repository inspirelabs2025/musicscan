...
export function EditScanModal({ scan, isOpen, onClose, onSuccess }: EditScanModalProps) {
  const [formData, setFormData] = useState({
    ...
    discogs_id: scan?.discogs_id?.toString() || "",
    ...
    try {
      const updateData: any = {
        ...
        manual_edits: {
          ...
          edited_fields: Object.keys(formData).filter(key => 
            formData[key as keyof typeof formData] !== scan[key as keyof AIScanResult]?.toString()
          ),
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Scan bewerken</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Album/release titel"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="catalog_number">Catalog nummer</Label>
              <Input
                id="catalog_number"
                value={formData.catalog_number}
                onChange={(e) => handleInputChange("catalog_number", e.target.value)}
                placeholder="Catalog nummer"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={formData.media_type} onValueChange={(value) => handleInputChange("media_type", value)}>
                <SelectTrigger>
                  <SelectValue placeholder="Selecteer media type" />
                </SelectTrigger>
                <SelectContent>
                  {mediaTypes.map((type) => (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleInputChange("discogs_id", e.target.value)}
          ...
          <div>
            <Label htmlFor="discogs_url">Discogs URL</Label>
            <Input
              id="discogs_url"
              type="url"
              value={formData.discogs_url}
              onChange={(e) => handleInputChange("discogs_url", e.target.value)}
