...
const useLatestAlbums = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const cdAlbums: Album[] = (cdData || []).map(item => ({
        id: item.id,
        artist: item.artist,
        title: item.title,
        label: item.label,
      ...
      const vinylAlbums: Album[] = (vinylData || []).map(item => ({
        id: item.id,
        artist: item.artist,
        title: item.title,
        label: item.label,
      ...
      const allAlbums = [...cdAlbums, ...vinylAlbums]
        ...
        .slice(0, 6); // Show 6 latest albums
...
const AlbumCard: React.FC<{ album: Album; index: number }> = ({ album, index }) => {
  const uploadDate = new Date(album.created_at).toLocaleDateString('nl-NL', {
    day: 'numeric',
    month: 'short'
  });
        ...
        <div className="relative z-10">
          ...
          <div className="w-full aspect-square mb-3 bg-gradient-to-br from-muted to-muted/50 rounded-lg overflow-hidden group-hover:scale-105 transition-all duration-300">
            {album.image_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-vinyl-purple/20 to-vinyl-gold/20">
                {mediaIcon}
              </div>
            )}
          </div>
          ...
          <div className="space-y-1">
            <div className="text-sm font-medium text-foreground/90 group-hover:text-vinyl-purple transition-colors line-clamp-1">
              {album.title}
            </div>
            <div className="text-xs text-muted-foreground group-hover:text-foreground/70 transition-colors line-clamp-1">
              {album.artist}
            </div>
            {album.year && (
            ...
            {/* Upload Date and Media Type */}
            <div className="flex items-center justify-between pt-2">
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                <Calendar className="w-3 h-3" />
                <span>{uploadDate}</span>
              </div>
              <div className={`flex items-center gap-1 text-xs ${mediaColor}`}>
                {mediaIcon}
                <span className="capitalize">{album.media_type}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 relative">
        <div className="text-center mb-8 animate-fade-in">
          ...
          <p className="text-muted-foreground">
            Ontdek de laatste albums die onze community heeft toegevoegd! 🆕
          </p>
        </div>
        ...
        <div className="text-center mt-6 animate-fade-in delay-300">
          <p className="text-sm text-muted-foreground">
            🎼 Voeg jouw collectie toe en inspireer anderen! ✨
          </p>
        </div>
      </div>
    </section>
