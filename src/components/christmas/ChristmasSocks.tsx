  ...
  const { data: socks, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('album_socks')
        .select('id, artist_name, album_title, slug, album_cover_url, mockup_url')
    ...
    <Card className="bg-card/80 backdrop-blur-sm border-border/50 overflow-hidden">
      ...
      <CardHeader className="relative">
        ...
        <p className="text-base text-muted-foreground mt-2">
          <span className="font-semibold text-foreground">Unieke muziek sokken</span> — het perfecte kerstcadeau voor muziekliefhebbers
        </p>
      </CardHeader>
                ...
                <div className="group bg-card rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 border border-border/50">
                  ...
                  <div className="p-4 space-y-2">
                    <div className="flex items-center gap-2 text-green-600">
                      ...
                      <span className="text-xs font-bold uppercase tracking-wide">Sokken</span>
                    </div>
                    <h3 className="font-semibold text-foreground line-clamp-1">
                      {sock.artist_name}
                    </h3>
                    <p className="text-sm text-muted-foreground">€24,95</p>
