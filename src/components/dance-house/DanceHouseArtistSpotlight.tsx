...
export const DanceHouseArtistSpotlight = () => {
      ...
      <section className="py-16 bg-gradient-to-r from-cyan-950/30 to-purple-950/30">
        <div className="container mx-auto px-4">
          <Card className="overflow-hidden bg-background/50 backdrop-blur">
            <div className="grid md:grid-cols-2 gap-6"><Skeleton className="h-80" /><div className="p-6 space-y-4"><Skeleton className="h-8 w-48" /><Skeleton className="h-24 w-full" /></div></div>
          </Card>
        </div>
      </section>
    );
  ...
  const defaultSpotlight = {
    ...
    biography: 'Het Franse duo Thomas Bangalter en Guy-Manuel de Homem-Christo revolutioneerde elektronische muziek.',
    slug: 'daft-punk'
  };
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/20 rounded-full mb-4">
            ...
            <span className="text-amber-400 text-sm">{ch.artistSpotlight}</span>
          </div>
          <h2 className="text-3xl font-bold">{ch.legendaryDanceArtist}</h2>
        </div>

        <Card className="overflow-hidden bg-background/50 backdrop-blur border-cyan-500/20 max-w-4xl mx-auto">
          <div className="grid md:grid-cols-2 gap-0">
            <div className="relative h-64 md:h-auto">
              <img src={artist.artwork_url || '/placeholder.svg'} alt={artist.artist_name} className="w-full h-full object-cover" />
            ...
            <CardContent className="p-6 flex flex-col justify-center">
              <h3 className="text-3xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400">{artist.artist_name}</h3>
              <p className="text-muted-foreground mb-6 line-clamp-4">{artist.biography || artist.story_content?.substring(0, 300) + '...'}</p>
              <div className="flex gap-3">
                <Button asChild className="bg-gradient-to-r from-cyan-500 to-purple-500">
                  <Link to={`/artists/${artist.slug}`}>{ch.readMore}<ExternalLink className="w-4 h-4 ml-2" /></Link>
                </Button>
              </div>
            </CardContent>
          </div>
