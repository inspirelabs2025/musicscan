...
const transformToUnifiedItems = (spotifyReleases: any[], newsBlogs: any[], userBlogs: any[], musicStories: any[]): UnifiedContentItem[] => {
  ...
  spotifyReleases.slice(0, 3).forEach((r) => items.push({ id: `release-${r.id}`, type: 'release', title: r.name, subtitle: r.artist, date: r.release_date, image: r.image_url, link: r.spotify_url, isExternal: true, badge: { label: '🎵 Spotify', className: 'bg-green-500 text-white' } }));
  newsBlogs.forEach((p: any) => items.push({ id: `news-${p.id}`, type: 'news', title: p.title, subtitle: p.source, date: p.published_at, image: p.image_url, link: `/nieuws/${p.slug}`, badge: { label: '📰 Nieuws', className: 'bg-blue-500 text-white' } }));
  userBlogs.forEach((p: any) => items.push({ id: `album-${p.id}`, type: 'album-story', title: `${p.yaml_frontmatter?.artist || 'Onbekend'} - ${p.yaml_frontmatter?.title || 'Album'}`, date: p.published_at, link: `/plaat-verhaal/${p.slug}`, badge: { label: '💿 Album', className: 'bg-purple-500 text-white' } }));
  musicStories.forEach((s: any) => items.push({ id: `single-${s.id}`, type: 'single-story', title: s.single_name ? `${s.artist_name} - ${s.single_name}` : s.title, date: s.created_at, image: s.artwork_url, link: `/muziek-verhaal/${s.slug}`, badge: { label: '🎵 Single', className: 'bg-amber-500 text-white' } }));
...
export const UnifiedContentWidget = () => {
  ...
  if (l1 || l2) return <Card className="border-2"><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-primary animate-pulse" />🌟 Ontdek & Leer</CardTitle></CardHeader><CardContent><div className="space-y-3">{[1,2,3,4,5,6].map(i => <div key={i} className="flex gap-3 p-3 border rounded-lg"><Skeleton className="w-16 h-16 rounded-lg" /><div className="flex-1 space-y-2"><Skeleton className="h-4 w-3/4" /><Skeleton className="h-3 w-1/2" /></div></div>)}</div></CardContent></Card>;
  ...
  return (
    <Card className="border-2 hover:border-primary/50 transition-all duration-300 hover:shadow-lg">
      <CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="w-5 h-5 text-primary animate-pulse" />🌟 Ontdek & Leer</CardTitle></CardHeader>
      <CardContent>
        {items.length > 0 ? (
          <div className="space-y-3">
            ...
            {items.length > 6 && <Button onClick={() => setShowAll(!showAll)} variant="outline" className="w-full">{showAll ? 'Toon minder' : `Toon meer (${items.length - 6} extra)`}</Button>}
            <div className="grid grid-cols-2 gap-2 pt-2">
              <Button asChild size="sm" variant="outline"><Link to="/nieuws"><Newspaper className="w-4 h-4 mr-2" />Nieuws<ArrowRight className="w-4 h-4 ml-2" /></Link></Button>
              <Button asChild size="sm" variant="outline"><Link to="/verhalen"><BookOpen className="w-4 h-4 mr-2" />Verhalen<ArrowRight className="w-4 h-4 ml-2" /></Link></Button>
            </div>
          </div>
        ) : <div className="text-center py-6"><Sparkles className="w-12 h-12 text-muted-foreground mx-auto mb-2 opacity-50" /><p className="text-sm text-muted-foreground">Geen content beschikbaar</p></div>}
      </CardContent>
    </Card>
  );
};
