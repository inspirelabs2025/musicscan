      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500/20 rounded-full mb-4">
            ...
            <span className="text-amber-400 text-sm">{fm.completeTimeline}</span>
          </div>
          <h2 className="text-3xl font-bold text-white">{fm.filmMusicHistory}</h2>
          <p className="text-white/60 mt-2">{fm.historySubtitle}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mb-8">
          {decades.map((decade) => (
            ...
            >
              {decade === 'all' ? fm.all : decade === '00s' ? '2000s' : decade === '10s' ? '2010s' : decade === '20s' ? '2020s' : `19${decade.slice(0, 2)}`}
            </Button>
          ))}
          ...
          >
            ...
            {sortOrder === 'asc' ? fm.oldestFirst : fm.newestFirst}
          </Button>
        </div>
        ...
        <div className="relative">
          ...
          <div className="space-y-6">
            {displayFeiten.map((feit, index) => (
              <div key={feit.slug} className={`relative flex items-center gap-8 ${index % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                  ...
                  <Link to={`/filmmuziek/feit/${feit.slug}`} className="group block">
                    <div className="bg-white/5 backdrop-blur rounded-xl p-5 border border-amber-500/20 hover:border-amber-500/40 transition-all inline-block max-w-lg">
                      <div className="flex items-center gap-2 mb-2 text-amber-400 font-bold">
                        ...
                        {feit.subgenre && <span className="text-xs px-2 py-1 bg-amber-500/10 rounded">{feit.subgenre}</span>}
                      </div>
                      <h3 className="text-white font-semibold mb-2 group-hover:text-amber-400 transition-colors">{feit.title}</h3>
                      ...
                      <div className="mt-3 flex items-center gap-1 text-amber-400 text-sm opacity-0 group-hover:opacity-100 transition-opacity">
                        {fm.readMore} <ChevronRight className="w-4 h-4" />
                      </div>
                    </div>
                  </Link>
                </div>
          ...
          <div className="text-center mt-8">
            <Link to="/filmmuziek/tijdlijn">
              <Button variant="outline" className="border-amber-500/30 text-amber-400 hover:bg-amber-500/10">
                {fm.viewAllMilestones} {filteredFeiten.length} {fm.milestones.toLowerCase()}
