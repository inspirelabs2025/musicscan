...
export const CommunityStats = () => {
  ...
  const count = stats?.scans ? Math.floor(stats.scans / 100) * 100 : 5000;
      ...
      <div className="container mx-auto px-4">
        <div className="text-center space-y-6">
          <h2 className="text-3xl md:text-4xl font-bold">
            {h.joinLovers.replace('{count}', count.toLocaleString())}
          </h2>
          <div className="flex flex-wrap justify-center gap-8 text-muted-foreground">
            <div className="text-center">
              <p className="text-3xl md:text-4xl font-bold text-foreground">{stats?.scans?.toLocaleString() || '10.000+'}</p>
              <p className="text-sm md:text-base">{h.albumsScanned}</p>
            </div>
            <div className="text-center">
              <p className="text-3xl md:text-4xl font-bold text-foreground">{stats?.stories?.toLocaleString() || '500+'}</p>
              <p className="text-sm md:text-base">{h.storiesLabel}</p>
            </div>
            <div className="text-center">
              <p className="text-3xl md:text-4xl font-bold text-foreground">{stats?.shops?.toLocaleString() || '50+'}</p>
              <p className="text-sm md:text-base">{h.activeShops}</p>
            </div>
          </div>
        </div>
      </div>
