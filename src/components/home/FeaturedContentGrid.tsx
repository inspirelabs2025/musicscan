    ...
    <div className="grid md:grid-cols-3 gap-6">
      ...
      <Card className="relative overflow-hidden group hover:border-primary/50 transition-all">
        {storyLoading ? (
          ...
          <>
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground line-clamp-3 mb-4">
                {story.meta_description || 'Ontdek het verhaal achter deze iconische plaat...'}
              </p>
              <Button asChild variant="outline" className="w-full group/btn">
                <Link to={`/muziek-verhaal/${story.slug}`}>
                  Lees het Verhaal
          ...
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">Geen verhaal beschikbaar</p>
          </CardContent>
        )}
      </Card>
      ...
      <Card className="relative overflow-hidden group hover:border-vinyl-gold/50 transition-all">
        {topicLoading ? (
          ...
          <>
            <CardHeader>
              <div className="flex items-center gap-2 mb-2">
                <Badge className="gap-1 bg-vinyl-gold/10 text-vinyl-gold border-vinyl-gold/20">
              ...
              <CardTitle className="line-clamp-2 group-hover:text-vinyl-gold transition-colors">
                {topic.title}
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">Geen discussie beschikbaar</p>
          </CardContent>
        )}
      </Card>
      ...
      <Card className="relative overflow-hidden group hover:border-accent/50 transition-all">
        {productLoading ? (
            ...
            <CardHeader>
              <div className="flex items-center gap-2 mb-2">
                ...
                {product.created_at && new Date(product.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) && (
                  <Badge className="animate-pulse">Nieuw!</Badge>
                )}
              </div>
          ...
          <CardContent className="p-6">
            <p className="text-sm text-muted-foreground">Geen product beschikbaar</p>
          </CardContent>
        )}
      </Card>
    </div>
