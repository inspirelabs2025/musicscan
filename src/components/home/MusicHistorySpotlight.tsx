...
export const MusicHistorySpotlight = () => {
  ...
  const { data: events, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('🎵 Fetching music history for:', dayOfMonth, monthOfYear);
      
      // Fetch from music_history_events table
      ...
      if (data && data.events) {
        const eventList = data.events as MusicHistoryEvent[];
        console.log('✅ Music history events for today:', eventList.length, 'events found');
        return { events: eventList, isToday: true };
      }
      
      // Fallback: get most recent day's events
      ...
      const recentEvents = recentData?.events as MusicHistoryEvent[] || [];
      console.log('✅ Recent music history events:', recentEvents.length, 'events found');
      return { events: recentEvents, isToday: false };
    }
  });
                ...
                <CarouselItem key={`${event.year}-${index}`} className="pl-2 basis-[75%]">
                  ...
                  >
                    <div className="p-4 bg-gradient-to-br from-primary/10 to-primary/5">
                      ...
                      {event.artist && (
                        <p className="text-xs text-muted-foreground line-clamp-1">{event.artist}</p>
                      )}
    ...
    <section className="container mx-auto px-4 py-12">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          {isToday ? 'Ontdek wat er op deze dag in de muziekgeschiedenis gebeurde' : 'Ontdek iconische momenten uit de muziekgeschiedenis'}
        </p>
      </div>
        ...
        {previewEvents.map((event, index) => (
          <Card key={`${event.year}-${index}`} className="overflow-hidden hover:shadow-lg transition-shadow">
            <CardContent className="p-6">
              ...
              {event.artist && (
                <p className="text-sm text-primary mb-2">{event.artist}</p>
              )}
