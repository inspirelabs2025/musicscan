...
export const PublicScannerSpotlight = () => {
  ...
  const features = [
    { icon: Disc3, label: h.vinylAnalysis },
    { icon: Camera, label: h.cdRecognition },
    { icon: DollarSign, label: h.priceCheckBadge },
    { icon: Globe, label: h.freeAndPublic },
  ];
        ...
        <div className="bg-gradient-to-br from-card to-muted/80 backdrop-blur-sm rounded-2xl p-6 md:p-8 shadow-xl border border-red-500/20">
          <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
            ...
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-3">{h.discoverValue}</h2>
              <p className="text-base text-muted-foreground mb-5 max-w-lg">{h.scanFreeDesc}</p>
              <div className="flex flex-wrap justify-center md:justify-start gap-2">
                {features.map((feature) => (
                  <Badge key={feature.label} variant="secondary" className="flex items-center gap-2 px-3 py-1.5 text-sm">
