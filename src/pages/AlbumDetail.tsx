...
export default function AlbumDetail() {
  ...
  const getAlbumImages = () => {
    ...
    if (album.media_type === 'cd') {
      if (album.front_image) images.push({ url: album.front_image, label: dp.front, type: "front", alt: generateAltTag(album.artist, album.title, album.year, 'CD', 'front') });
      if (album.back_image) images.push({ url: album.back_image, label: dp.backSide, type: "back", alt: generateAltTag(album.artist, album.title, album.year, 'CD', 'back') });
      if (album.barcode_image) images.push({ url: album.barcode_image, label: "Barcode", type: "barcode", alt: generateAltTag(album.artist, album.title, album.year, 'CD', 'barcode') });
      if (album.matrix_image) images.push({ url: album.matrix_image, label: "Matrix", type: "matrix", alt: generateAltTag(album.artist, album.title, album.year, 'CD', 'matrix') });
    } else {
      if (album.catalog_image) images.push({ url: album.catalog_image, label: dp.catalog, type: "catalog", alt: generateAltTag(album.artist, album.title, album.year, 'Vinyl', 'catalog') });
      if (album.matrix_image) images.push({ url: album.matrix_image, label: "Matrix", type: "matrix", alt: generateAltTag(album.artist, album.title, album.year, 'Vinyl', 'matrix') });
      if (album.additional_image) images.push({ url: album.additional_image, label: dp.extra, type: "additional", alt: generateAltTag(album.artist, album.title, album.year, 'Vinyl', 'additional') });
    }
    return images;
  };
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-4xl mx-auto">
            ...
            <Card className="p-8 text-center">
              <h1 className="text-2xl font-bold mb-4">{dp.albumNotFound}</h1>
              <p className="text-muted-foreground">{dp.albumNotFoundDesc}</p>
            </Card>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-vinyl-purple/5 via-transparent to-primary/5">
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-7xl mx-auto">
            <Button variant="ghost" onClick={() => navigate(-1)} className="mb-6 hover:bg-accent/50">
              <ArrowLeft className="w-4 h-4 mr-2" />
              {dp.backToCollection}
            </Button>
              ...
              <div className="lg:col-span-1 space-y-6">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    {album.is_public && <Badge variant="default">{dp.publicBadge}</Badge>}
                    {album.is_for_sale && <Badge variant="secondary">{dp.forSale}</Badge>}
                    {album.condition_grade && <Badge variant="outline">{album.condition_grade}</Badge>}
                  ...
                  <div className="grid grid-cols-1 gap-3 text-sm">
                    ...
                    {album.year && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{dp.year}</span>
                    ...
                    {album.catalog_number && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{dp.catalogNumber}</span>
                        <span className="font-medium">{album.catalog_number}</span>
                      </div>
                    )}
                    {album.genre && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{dp.genre}</span>
                        <span className="font-medium flex items-center gap-1">
                    ...
                    {album.country && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{dp.country}</span>
                  ...
                  {album.shop_description && (
                    <div className="mt-4 p-4 bg-muted/50 rounded-lg">
                      <h3 className="font-medium mb-2">{dp.description}</h3>
                  ...
                  <Card className="mt-4">
                    ...
                    <CardContent className="text-xs space-y-2">
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{dp.addedOn}</span>
                        <span>{format(new Date(album.created_at), "dd MMM yyyy, HH:mm", { locale: dateFnsLocale })}</span>
                      </div>
                      {album.updated_at && album.updated_at !== album.created_at && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">{dp.updatedOn}</span>
                          <span>{format(new Date(album.updated_at), "dd MMM yyyy, HH:mm", { locale: dateFnsLocale })}</span>
                        </div>
                      )}
                    </CardContent>
                  </Card>
              ...
              <>
                ...
                <Card className="p-8">
                  <div className="flex items-center justify-center space-x-2">
                    ...
                    <span className="text-muted-foreground">{dp.generatingInsights}</span>
                  </div>
                </Card>
              </>
            )}
