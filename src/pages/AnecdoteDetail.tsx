        ...
        <div className="min-h-screen flex items-center justify-center">
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-lg text-muted-foreground">{dp.anecdoteNotFound}</p>
    ...
    <>
      ...
      <div className="min-h-screen bg-background">
        <BreadcrumbNavigation
          items={[
            { name: 'Home', url: '/' },
            { name: tr.nav.anecdotes, url: '/anekdotes' },
            { name: anecdote.anecdote_title, url: `/anekdotes/${slug}` },
          ]}
        />

        <div className="container mx-auto px-4 py-8 max-w-4xl">
          <Button variant="ghost" onClick={() => navigate('/anekdotes')} className="mb-6">
          ...
          <Card className="mb-8">
            <CardContent className="pt-6">
              <div className="flex items-center gap-2 mb-4 flex-wrap">
                <Badge variant="outline">{anecdote.subject_type}</Badge>
                <Badge variant="secondary">{anecdote.subject_name}</Badge>
              </div>

              <h1 className="text-3xl md:text-4xl font-bold mb-4">{anecdote.anecdote_title}</h1>

              <div className="flex items-center gap-4 text-sm text-muted-foreground flex-wrap mb-6">
                <div className="flex items-center gap-1">
                  <Calendar className="w-4 h-4" />
                  {format(new Date(anecdote.anecdote_date), 'd MMMM yyyy', { locale: dateFnsLocale })}
                </div>
                <div className="flex items-center gap-1">
                  <Eye className="w-4 h-4" />
                  {anecdote.views_count || 0} {dp.views}
                </div>
                <div className="flex items-center gap-1">
                  <BookOpen className="w-4 h-4" />
                  {anecdote.reading_time || 1} min {dp.readingTime}
                </div>
              </div>
            ...
            <CardContent className="pt-6">
              ...
              {anecdote.source_reference && (
                <div className="mt-8 p-4 bg-muted rounded-lg text-sm">
                  <strong>{dp.sourceRef}</strong> {anecdote.source_reference}
                </div>
              )}
            </CardContent>
          </Card>
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {relatedAnecdotes.map((related) => (
                  ...
                  >
                    <CardContent className="pt-6">
                      <Badge variant="outline" className="mb-2">{related.subject_type}</Badge>
                      <h3 className="font-semibold mb-2 line-clamp-2">{related.anecdote_title}</h3>
