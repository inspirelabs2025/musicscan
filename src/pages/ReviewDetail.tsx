      ...
      <>
        ...
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="text-center space-y-4">
            ...
            <Link to="/reviews">
              <Button>{dp.backToOverview}</Button>
            </Link>
          </div>
        </div>
      </>
      ...
      <article className="min-h-screen bg-background">
            ...
            <div className="sticky top-8">
              <div className="aspect-square rounded-lg overflow-hidden shadow-2xl bg-muted">
                {review.cover_image_url ? (
                  <img src={review.cover_image_url} alt={`${review.artist_name} - ${review.album_title}`} className="w-full h-full object-cover" />
                ) : (
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {review.genre && (
                    <Badge variant="secondary" className="gap-1"><Tag className="h-3 w-3" />{review.genre}</Badge>
                  )}
                  {review.format && <Badge variant="outline">{review.format.toUpperCase()}</Badge>}
                  {review.release_year && (
                    <Badge variant="outline" className="gap-1"><Calendar className="h-3 w-3" />{review.release_year}</Badge>
                  )}
                  {review.label && <Badge variant="outline">{review.label}</Badge>}
                </div>

                {review.author_name && (
                  <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
                    <User className="h-4 w-4" />
                    <span>{dp.by} {review.author_name}</span>
                  </div>
                )}

                <p className="text-lg text-muted-foreground leading-relaxed">{review.summary}</p>
              </div>

              {review.rating_breakdown && <RatingBreakdown breakdown={review.rating_breakdown} />}

              {review.listening_context && (
                <div className="p-4 bg-muted/50 rounded-lg border-l-4 border-primary">
                  <p className="text-sm font-medium mb-1">{dp.listeningContext}</p>
        ...
        <div className="container mx-auto max-w-3xl px-4 py-8">
          ...
          {review.spotify_embed_url && (
            <div className="mt-12">
              <h3 className="text-xl font-semibold mb-4">{dp.listenAlbum}</h3>
              <iframe src={review.spotify_embed_url} width="100%" height="380" frameBorder="0" allow="encrypted-media" className="rounded-lg" />
            </div>
          )}

          {review.youtube_embed_url && (
            <div className="mt-12">
              <h3 className="text-xl font-semibold mb-4">{dp.video}</h3>
              <div className="aspect-video rounded-lg overflow-hidden">
                <iframe src={review.youtube_embed_url} width="100%" height="100%" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen />
              </div>
            </div>
          )}
          ...
          {review.recommended_for && (
            <div className="mt-12 p-6 bg-primary/5 rounded-lg border">
              <h3 className="text-xl font-semibold mb-3">{dp.recommendedFor}</h3>
