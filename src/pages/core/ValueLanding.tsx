...
export default function ValueLanding() {
  ...
  const howToSchema = {
    ...
    url: `${SITE_URL}${corePath('value', locale)}`,
  ...
  const icons = [ScanLine, Search, TrendingUp];
        ...
        <header className="text-center mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-8">
            <Button asChild size="lg">
              <Link to="/quick-price-check">{copy.ctaPrimary}</Link>
            </Button>
            <Button asChild size="lg" variant="outline">
              <Link to={corePath('scan', locale)}>{copy.ctaSecondary}</Link>
            </Button>
          </div>
        </header>
