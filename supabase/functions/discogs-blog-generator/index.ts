...
serve(async (req) => {
  ...
  try {
    console.log('🚀 Discogs blog generator triggered');
    ...
    for (const release of newReleases.slice(0, 5)) { // Process max 5 at a time
      try {
        console.log(`📝 Processing: ${release.artist} - ${release.title}`);
        ...
        if (existingBlog) {
          console.log(`⏭️ Blog already exists for ${release.title}`);
          continue;
        }
        ...
        const { data: blogResult, error: blogError } = await supabase.functions.invoke('plaat-verhaal-generator', {
          body: {
            albumId: releaseId,
            albumType: 'release', // Use release type for Discogs releases
        ...
        if (blogError) {
          console.warn(`⚠️ Blog generation failed for ${release.title}:`, blogError);
          errors++;
        } else {
        ...
        console.error(`❌ Error processing ${release.title}:`, error);
        errors++;
      }
    }

    console.log(`✅ Discogs blog generation complete: ${processed} processed, ${errors} errors`);
    ...
    console.error('❌ Error in discogs-blog-generator:', error);
    return new Response(JSON.stringify({
      success: false,
      error: error.message
    }), {
