...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Insert new releases into tracking table and create releases + blogs
    if (newReleasesToTrack.length > 0) {
      console.log(`📝 Tracking ${newReleasesToTrack.length} new releases...`);
      
      // Track releases
      const { error: trackingError } = await supabase
        .from('discogs_releases_shown')
        .insert(newReleasesToTrack);
      
      if (trackingError) {
        console.warn('⚠️ Failed to track releases:', trackingError);
      } else {
        console.log('✅ Successfully tracked new releases');
      }

      // Create releases and generate blogs in background
      const backgroundBlogTasks = async () => {
        for (const releaseToTrack of newReleasesToTrack) {
          try {
            const release = deduplicatedReleases.find(r => r.id === releaseToTrack.discogs_id);
            if (!release) continue;

            console.log(`📀 Creating release for: ${release.artist} - ${release.title}`);
            
            // Create release using the existing function
            ...
            if (releaseError) {
              console.warn(`⚠️ Failed to create release for ${release.title}:`, releaseError);
              continue;
            }

            const releaseId = releaseResult?.release_id;
            if (!releaseId) {
              console.warn(`⚠️ No release_id returned for ${release.title}`);
              continue;
            }

            console.log(`✅ Release created: ${releaseId} for ${release.title}`);

            // Generate blog for this release
            console.log(`📝 Generating blog for: ${release.artist} - ${release.title}`);
            
            // Determine album type based on format
            let albumType = 'vinyl'; // Default to vinyl for Discogs releases
            if (release.format) {
              const formatStr = Array.isArray(release.format) ? release.format.join(' ') : String(release.format);
              if (formatStr.toLowerCase().includes('cd')) {
                albumType = 'cd';
              }
            }
            ...
            if (blogError) {
              console.warn(`⚠️ Failed to generate blog for ${release.title}:`, blogError);
            } else {
              console.log(`✅ Blog generated for: ${release.title}`);
            }

          } catch (error) {
            console.error(`❌ Background task failed for release ${releaseToTrack.discogs_id}:`, error);
          }
        }
      };

      // Execute background tasks
      backgroundBlogTasks().catch(error => {
