...
serve(async (req) => {
  ...
  try {
    ...
    const podcastSlug = url.searchParams.get("podcast") || url.pathname.split("/").pop();

    if (!podcastSlug) {
      return new Response("Missing podcast parameter", { status: 400 });
    }
...
<rss version="2.0" 
  ...
  <channel>
    ...
    <item>
      ...
      ${episode.artwork_url ? `<itunes:image href="${episode.artwork_url}"/>` : ""}
    </item>`).join("\n")}
  </channel>
</rss>`;
