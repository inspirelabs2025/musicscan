-- Create trigger function to automatically delete cd_scan records with NULL calculated_advice_price
...
-- Create AFTER INSERT trigger on cd_scan table
CREATE TRIGGER trigger_cleanup_cd_scan_null_advice_price
  AFTER INSERT ON public.cd_scan
  FOR EACH ROW
  EXECUTE FUNCTION public.cleanup_cd_scan_null_advice_price();

-- Also create AFTER UPDATE trigger to handle updates that set calculated_advice_price to NULL
CREATE TRIGGER trigger_cleanup_cd_scan_null_advice_price_update
