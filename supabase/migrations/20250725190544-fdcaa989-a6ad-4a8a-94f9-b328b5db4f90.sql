...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vinyl_records_user_discogs 
ON vinyl_records(user_id, discogs_id) WHERE discogs_id IS NOT NULL;

-- Index for vehicle_parts by user_id and category
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vehicle_parts_user_category 
ON vehicle_parts(user_id, category_id);

-- Index for part_scans by user_id and status
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_part_scans_user_status 
