-- Add unique constraints for Spotify tables to enable proper upserts (using correct column names)

-- Add unique constraint for spotify_playlists (user_id + spotify_playlist_id combination)
ALTER TABLE spotify_playlists 
ADD CONSTRAINT unique_user_spotify_playlist 
UNIQUE (user_id, spotify_playlist_id);

-- Add unique constraint for spotify_tracks (user_id + spotify_track_id combination)  
