...
DROP FUNCTION IF EXISTS get_all_queue_stats();
DROP FUNCTION IF EXISTS get_output_totals(date, date);

-- Recreate get_cronjob_output_stats with correct column names
...
BEGIN
  ...
  SELECT * FROM (
    ...
    -- Anecdotes (uses is_active not is_published)
    SELECT 
...
-- Recreate get_all_queue_stats without ORDER BY in UNION
...
BEGIN
  ...
  SELECT * FROM (
    -- Batch Queue Items
    SELECT 
      'batch_queue_items'::text as queue_name,
      ...
      ROUND(COUNT(*) FILTER (WHERE status = 'completed' AND processed_at > now() - interval '24 hours')::numeric / 24, 1) as items_per_hour
...
BEGIN
  RETURN QUERY
  SELECT 
    COALESCE(SUM(items_created), 0)::bigint as total_created,
    COALESCE(SUM(items_posted), 0)::bigint as total_posted,
    (SELECT COALESCE(SUM(pending), 0)::bigint FROM get_all_queue_stats()) as total_in_queue,
    COALESCE(SUM(items_failed), 0)::bigint as total_failed
  FROM get_cronjob_output_stats(p_start_date, p_end_date);
END;
$$;