-- Fix get_cronjob_output_stats - news_blog_posts uses published_at not is_published
...
BEGIN
  ...
  SELECT * FROM (
    ...
    -- News Blog Posts (uses published_at IS NOT NULL for published status)
    SELECT 
