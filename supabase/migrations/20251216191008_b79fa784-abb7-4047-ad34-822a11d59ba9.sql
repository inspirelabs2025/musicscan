-- Update get_cronjob_last_output to properly count platform_products for process-discogs-queue
...
BEGIN
  ...
  -- daily-youtube-discoveries -> youtube_music_videos
  SELECT 
    'daily-youtube-discoveries'::text,
    'youtube_music_videos'::text,
    max(created_at),
    count(*)::bigint
  FROM youtube_music_videos
  WHERE created_at >= today_start AND created_at < today_end
