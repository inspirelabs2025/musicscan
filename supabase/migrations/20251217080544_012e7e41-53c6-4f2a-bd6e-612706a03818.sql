-- Add generate-static-sitemaps to get_cronjob_last_output
...
BEGIN
  ...
  UNION ALL
  -- sitemap_regeneration_log for generate-static-sitemaps (triggered by queue processor)
  SELECT 'generate-static-sitemaps'::text, 'sitemap_regeneration_log'::text,
         MAX(started_at), COUNT(*) FILTER (WHERE DATE(started_at) = CURRENT_DATE)
  FROM sitemap_regeneration_log;
END;
$function$;